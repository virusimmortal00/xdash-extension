export function selectImage(url: string, isCustom: boolean = false): void {
    ...
    const customImagePreview: HTMLElement | null = document.getElementById('customImagePreview');
    ...
    if (isCustom && customImagePreview) {
        ...
        const predefinedImage: HTMLImageElement | null = document.querySelector(`.image-option[src="${url}"]`);
        if (predefinedImage) {
